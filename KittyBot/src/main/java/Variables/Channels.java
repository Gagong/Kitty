package Variables;

public class Channels {
    //INFORMATION
    private String rules = "606543082653089806";
    private String news = "598179332178968594";
    private String ingameNews = "606545831859847189";
    private String marketplace = "662703338453729290";

    //DOPE DEVELOPMENT
    private String requestedFeaturesD = "607651000819646491";
    private String featuresRequestsD = "655676998286442527";

    //PERKAVA DEVELOPMENT
    private String requestedFeaturesP = "623446782155685899";
    private String featuresRequestsP = "623446817757069312";

    //SUPPORT
    private String windowsBotGuide = "606533259941380106";
    private String linuxBotGuide = "660415577386647552";
    private String paymentMethods = "626359485790683156";
    private String profileTemplates = "626653618644451338";
    private String support = "656178853055561739";
    private String bugReports = "606538901242380318";

    //TALK ZONE
    private String en = "598182739228753941";
    private String es = "606510511227535360";
    private String pl = "606453590038544384";
    private String tr = "606453615191785502";
    private String czsk = "606453961104293908";
    private String ru = "606460360651833351";
    private String de = "606510673647632429";
    private String it = "615846054440468480";
    private String pt = "615846402035154954";
    private String fr = "615846494712496128";
    private String hu = "615896506670776320";

    //COMMUNITY
    private String giveaway = "621301617785831431";
    private String translation = "615709320641773588";
    private String screenshots = "606416923403616266";
    private String videos = "606417989293572096";

    //DISCORD BOT
    private String botTalk = "609428100962582541";
    private String botRequests = "608313345866596370";
    private String ticketsArchive = "655729108789100565";
    private String warnedArchive = "663119990429581352";

    public String server = "598177730890039298";
    public String VPSserver = "637718328479186964";
    public String VPSManagment = "669641353575006220";

    public String getVPSManagment() {
        return VPSManagment;
    }

    public String getVPSserver() {
        return VPSserver;
    }

    public String getMarketplace() {
        return marketplace;
    }

    public String getLinuxBotGuide() {
        return linuxBotGuide;
    }

    public String getServer() {
        return server;
    }

    public String getWarnedArchive() {
        return warnedArchive;
    }

    public String getTicketsArchive() { return ticketsArchive; }

    public String getWindowsBotGuide() {
        return windowsBotGuide;
    }

    public String getBugReports() {
        return bugReports;
    }

    public String getEn() {
        return en;
    }

    public String getEs() {
        return es;
    }

    public String getFeaturesRequestsD() {
        return featuresRequestsD;
    }

    public String getFeaturesRequestsP() {
        return featuresRequestsP;
    }

    public String getCzsk() {
        return czsk;
    }

    public String getDe() {
        return de;
    }

    public String getIngameNews() {
        return ingameNews;
    }

    public String getNews() {
        return news;
    }

    public String getPaymentMethods() {
        return paymentMethods;
    }

    public String getPl() {
        return pl;
    }

    public String getIt() {
        return it;
    }

    public String getProfileTemplates() {
        return profileTemplates;
    }

    public String getRequestedFeaturesD() {
        return requestedFeaturesD;
    }

    public String getRequestedFeaturesP() {
        return requestedFeaturesP;
    }

    public String getRu() {
        return ru;
    }

    public String getRules() {
        return rules;
    }

    public String getSupport() {
        return support;
    }

    public String getFr() {
        return fr;
    }

    public String getPt() {
        return pt;
    }

    public String getTr() {
        return tr;
    }

    public String getBotRequests() {
        return botRequests;
    }

    public String getBotTalk() {
        return botTalk;
    }

    public String getGiveaway() {
        return giveaway;
    }

    public String getHu() {
        return hu;
    }

    public String getScreenshots() {
        return screenshots;
    }

    public String getTranslation() {
        return translation;
    }

    public String getVideos() {
        return videos;
    }
}
